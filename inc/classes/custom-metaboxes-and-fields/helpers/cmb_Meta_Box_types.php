...
class cmb_Meta_Box_types {
	...
	public function render_repeatable_field() {
		...
		<p class="add-row">
			<a data-selector="<?php echo $table_id; ?>" class="add-row-button button" href="#"><?php esc_html_e( 'Add Row', 'cmb' ); ?></a>
		</p>
		...
		if ( $meta_value && is_array( $meta_value ) ) {
			...
			foreach ( $meta_value as $id => $fullurl ) {
				...
				if ( $this->is_valid_img_ext( $fullurl ) ) {
					...
					echo
					'<li>',
						__( 'File:', 'cmb' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $fullurl, '" target="_blank" rel="external">'. esc_html__( 'Download', 'cmb' ) .'</a> / <a href="#" class="cmb_remove_file_button">'. esc_html__( 'Remove', 'cmb' ) .'</a>)
						'. $id_input .'
					</li>';
				}
			}
